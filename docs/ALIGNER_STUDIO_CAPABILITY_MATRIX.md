...
`IMPLEMENTED_BUT_NOT_PROVEN` + `ENVIRONMENT_BLOCKED` (local host) · External A100 Colab qualification demonstrated PyTorch CUDA + pointops against the pinned checkpoint. Repository preprocessing reproducibility with seed `123456` is proven by immutable `fv032_exact_repository_preprocessing_reproducibility.json`. FV-03.2 seals run contracts and evidence bundles with provenance; it does not claim clinical accuracy. On this host FV-03.1 self-test remains `ENVIRONMENT_UNAVAILABLE` with blockers `DRIVER_UNAVAILABLE`, `PYTORCH_UNAVAILABLE`, and `CUDA_EXTENSION_UNAVAILABLE` unless a clean CUDA machine is used. Checkpoint metadata is `TENSOR_CONTRACT_ESTABLISHED` and is not readiness. No fixture substitutes for inference. · `tests/python/test_fv03_segmentation.py`, `tests/python/test_fv03_1_runtime.py`, `tests/python/test_fv03_2_evidence.py`, `docs/FV03_SEGMENTATION.md`, `docs/FV03_1_SEGMENTATION_RUNTIME.md`, `docs/FV03_2_SEGMENTATION_EVIDENCE_GATE.md`, `.research/tmp/fv03_report.json`, `.research/tmp/fv03_1_report.json`, `fv032_exact_repository_preprocessing_reproducibility.json`. · A clean CUDA machine must execute the same accepted prepared artifact. A second real case is still required. · A1, A5, PyTorch CUDA, pointops, teethland source revision `424252e3d94a1565c8c2090eb5bb456b76386b93`. · An accepted prepared arch. · ToothInstanceNet behind the FV-03 backend interface. PyTorch is not an application startup dependency. Not production-ready on this host. · `READY_FOR_INFERENCE`, then a candidate whose prepared SHA, model SHA, and `clinical_accuracy_claim` false are stored, with no fabricated FDI. · FV-03 record: prepared load 145.5 ms, input gate 326.3 ms, capability detection 1511.3 ms, inference not attempted, peak RSS 529.3 MB. FV-03.1 appended a later probe: load 34.1 ms, gate 80.2 ms, capability 340.6 ms, inference not entered, peak RSS 421.0 MB. The later peak does not replace 529.3 MB. · Required. Not met on this host. · Not clinical accuracy. Not FDI. Seven classes are `NOT_ESTABLISHED`. Preprocessing reproducibility is not clinical accuracy. · Pinned SHA-256 `100c68a9b120402cc75539eff6347bd998bce8b1d4f01550638f471797d70803`.
...
`IMPLEMENTED_BUT_PARTIAL` · Accept, reject, hide, show, mark for review, merge, undo, redo, and reset write provenance on a candidate: previous state hash, affected instance ids, operation, parameters, timestamp, and a geometry hash when faces change. The model snapshot is not rewritten. Accept does not set `VERIFIED` and does not become clinical approval. Split is `SPLIT_UNAVAILABLE` because the form has no safe face partition. Manual segmentation correction is `NOT_IMPLEMENTED` and does not replace the model. These edits were tested with a deterministic mock because this host produced no instances. There is no brush. · `tests/python/test_fv03_segmentation.py`, `tests/python/test_fv03_1_runtime.py`, `tests/python/test_fv03_2_evidence.py`, `docs/FV03_1_SEGMENTATION_RUNTIME.md`, `docs/FV03_2_SEGMENTATION_EVIDENCE_GATE.md`. · Brush or boundary paint, and the same edits on a live ToothInstanceNet candidate. · B1. · Candidate face indices. · FV-03.1 / FV-03.2 review record. · A merge of overlapping faces is refused, and undo restores the recorded operation without changing the model snapshot. · Not timed on a real segmentation. · Required. · A doctor edit is not clinical verification and does not assign FDI. · Review operation, parent instance ids, prepared SHA, geometry hash.

### B5. Segmentation review
`IMPLEMENTED_BUT_PARTIAL` · The intake form shows model-derived segmentation, `NOT_ESTABLISHED` identity, `QUALITY_EVALUATION NOT_AVAILABLE`, validation status, evidence hash, execution origin, and either `ENVIRONMENT_BLOCKED` or `SEGMENTATION_COMPLETED`. External CUDA imports stay labeled `EXTERNAL_CUDA` and are not shown as local native execution. Model confidence is labeled model confidence, or `NOT_AVAILABLE`. Review controls render only when a completed result has `real_inference` true. Doctor acceptance is labeled as not clinical verification. The inspector shows status and does not start a job. The older analysis step still shows pipeline truth. · `SegmentationReviewForm.test.tsx`, `wave10AdaptiveInspector.test.tsx`, `tests/e2e/fv03_segmentation.spec.ts`, `docs/FV03_1_SEGMENTATION_RUNTIME.md`, `docs/FV03_2_SEGMENTATION_EVIDENCE_GATE.md`. · Review of a live candidate on two real cases. · B1, B4. · Segmentation candidate or an explicit blocker. · Existing inspector plus the FV-03 form. · A blocked run does not say the case is segmented or clinically verified. · Not separately timed beyond the capability probe. · Required on both real cases. · Review UI must not present a model class as FDI. · Run id, capability state, semantic identity, self-test state, evidence bundle SHA.
