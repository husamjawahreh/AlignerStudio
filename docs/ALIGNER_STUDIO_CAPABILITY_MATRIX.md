...
`IMPLEMENTED_BUT_NOT_PROVEN` + `ENVIRONMENT_BLOCKED` · Real uploads call ToothInstanceNet only when the runtime probe says the backend is executable. FV-03 refuses raw, stale, partial, and failed preparation inputs and persists the prepared SHA. FV-01.1 read the pinned checkpoint: DentalNet, 6 input channels, offsets/sigmas, one seed logit, 7 identify logits, instance ids from clustering. The FV-03 probe on this host is `DRIVER_UNAVAILABLE`, also `PYTORCH_UNAVAILABLE` and `CUDA_EXTENSION_UNAVAILABLE`. No inference ran. CPU fallback was ruled out. A blocked run stores the blocker and does not fabricate instances. · `tests/python/test_fv03_segmentation.py`, `docs/FV03_SEGMENTATION.md`, `.research/tmp/fv03_report.json`. · A clean CUDA machine must execute the same accepted prepared artifact. A second real case is still required. · A1, A5, PyTorch CUDA, pointops, teethland source revision `424252e3d94a1565c8c2090eb5bb456b76386b93`. · An accepted prepared arch. · ToothInstanceNet behind the FV-03 backend interface. Not production-ready on this host. · `AVAILABLE` then a candidate whose prepared SHA, model SHA, and `clinical_accuracy_claim` false are stored, with no fabricated FDI. · Prepared load 145.5 ms, input gate 326.3 ms, capability detection 1511.3 ms. Inference was not attempted. Peak RSS 529.3 MB in that process. · Required. Not met. · Not clinical accuracy. Not FDI. Seven classes are `NOT_ESTABLISHED`. · Pinned SHA-256 `100c68a9b120402cc75539eff6347bd998bce8b1d4f01550638f471797d70803`.
...
`IMPLEMENTED_BUT_PARTIAL` · Accept, reject, hide, show, mark for review, merge, split, undo, and reset write provenance on a candidate. The model snapshot is not rewritten. Accept does not set `VERIFIED`. Split requires an explicit face subset. These edits were tested with a deterministic mock because this host produced no instances. There is no brush. · `tests/python/test_fv03_segmentation.py`, `docs/FV03_SEGMENTATION.md`. · Brush or boundary paint, and the same edits on a live ToothInstanceNet candidate. · B1. · Candidate face indices. · FV-03 review record. · A merge of overlapping faces is refused, and undo restores the previous review state without changing the model snapshot. · Not timed on a real segmentation. · Required. · A doctor edit is not clinical verification and does not assign FDI. · Review event, parent instance ids, prepared SHA.

### B5. Segmentation review
`IMPLEMENTED_BUT_PARTIAL` · The intake form shows model-derived segmentation, `NOT_ESTABLISHED` identity, and a blocked runtime when the probe cannot run. Review controls stay on that form. The inspector shows status and does not start a job. The older analysis step still shows pipeline truth. · `SegmentationReviewForm.test.tsx`, `wave10AdaptiveInspector.test.tsx`, `docs/FV03_SEGMENTATION.md`. · Review of a live candidate on two real cases. · B1, B4. · Segmentation candidate or an explicit blocker. · Existing inspector plus the FV-03 form. · A blocked run does not say the case is segmented or clinically verified. · Not separately timed beyond the capability probe. · Required on both real cases. · Review UI must not present a model class as FDI. · Run id, capability state, semantic identity.
